...
<#
Function Parse-AppLockerPolicy {
    param (
        [xml] $AppLockerPolicy
    )
...
Function Parse-RuleCollection {
    ...
    foreach ($Rule in $Collection) {
        $Rules += Parse-Rule -Rule $Rule
    }

    $Enforcement = $RuleCollection.Node.EnforcementMode
...
Function Parse-Rule {
    ...
    return $Rule.Node.InnerXml.tostring()
}
